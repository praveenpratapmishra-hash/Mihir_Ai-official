...
st.markdown("""
    ...
    .footer-text {text-align: center; font-size: 15px; color: #888; font-weight: bold; margin-top: 10px;}
    
    /* Force Display Plus Icon */
    .stFileUploader {
...
# --- 2. THE ATTACH BUTTON (+) ---
# Maine iska label change kiya hai taaki Streamlit ise naya element samjhe
uploaded_file = st.file_uploader("➕ ATTACH PHOTO/DOC", type=['png', 'jpg', 'jpeg', 'pdf'])
...
if len(st.session_state.messages) == 0:
    ...
    with col2:
        if st.button("📚 Study Help"): st.session_state.temp = "Help me with my studies"
...
if st.session_state.p_limit >= 5:
    st.error("⚠️ 5 Photo Limit Reached! Watch Ad to Unlock.")
else:
    if "temp" in st.session_state: prompt = st.session_state.pop("temp")
    if prompt or uploaded_file:
        user_msg = prompt if prompt else "Analyze this file"
