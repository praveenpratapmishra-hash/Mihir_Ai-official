...
# --- 2. CSS (CLEAN LOOK & NO BRANDING) ---
st.markdown("""
    <style>
    /* GitHub/Streamlit Patti & Menu Hide */
    ...
    /* Input Bar & Plus Button placement */
...
# --- 4. TOP UI (Mihir AI + 4 Buttons) ---
if not st.session_state.messages:
    st.markdown("<h1 style='text-align: center; color: #4A90E2; margin-top: -30px;'>Mihir AI</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>Hello! Main aapki kya madad karun?</p>", unsafe_allow_html=True)
    
    # 4 Buttons in 2x2 Grid
    c1, c2 = st.columns(2)
    with c1:
        if st.button("🧠 Solve Problem"): st.session_state.temp = "Dost, solve this for me."
    ...
    with st.chat_message("assistant"):
        try:
            if any(x in (prompt or "").lower() for x in ["create", "photo", "banao", "image"]):
                ...
                if up_file:
                    res = model.generate_content([prompt if prompt else "Explain this", PIL.Image.open(up_file)])
                else:
